...
$StartOfMonth=$_GET['CM'];        //--   check the selected month via NEXT or PREVIOUS button by receiving url
if ($StartOfMonth=='')              //----------- if no url receive then check today's month
    {$day = date('w');
     $StartOfMonth=date('d-m-Y');
    }
    
$sqlStartOfMonth = date ("Y-m-d", strtotime($StartOfMonth));

$EndOfMonth=date('d-m-Y', strtotime('+30 days', strtotime($StartOfMonth)));
$sqlEndOfMonth = date ("Y-m-d", strtotime($EndOfMonth));
$NoOfDays=date("d", strtotime($EndOfMonth)); $NoOfDays+=1;                                                                        //--------------- calculate PREVIOUS & NEXT month
$pMonth=date ("d-m-Y", strtotime("-1 Month", strtotime($StartOfMonth)));
$nMonth=date ("d-m-Y", strtotime("+1 Month", strtotime($StartOfMonth)));    
...
$south.="<table class='table-list' >
        ...
        <span>$StartOfMonth </span>
        <a href=pcalendarm.php?CM=$nMonth target=_self> <pre style='display: inline-block;
...
$showdate=$StartOfMonth;
while (strtotime($showdate) <= strtotime($EndOfMonth))     //------ loop to show 15 days in header
    { 
...
if ($ForTaskTag!='') {$FTTagCriteria.=" AND t3.cRecRef IN ( SELECT cRecRef FROM `tTaskTags` WHERE TagTitle='$ForTaskTag'  AND RefUSR ='$id') ";}
    ...
    while($row101=mysqli_fetch_array($sql101))						//------------------- Store Practice ID & Full Name from database to AllPractice_arr ------
		...
		$query132="SELECT t1.cRecRef, t1.cScheduleDate, t1.cDueDate, t1.Stage, t1.CompleteBy, t2.SRecRef, t1.ForRefUSR, t2.ForCoRecRef, t3.TRecRef, t3.TaskTitle , t3.Priority
				   ...
				   AND t1.cDueDate<'$sqlStartOfMonth' AND t1.CompleteBy=0 and t1.Status='A'
        ...
        $sout.="</td>";
            ...
            $tTaskRefPrvDay ="";
            while (strtotime($showdate) <= strtotime($EndOfMonth))     //------ loop to show 30 days in header
                { 
